...
function rain(){
    ...
    let bufferSize = 2 * audioCtx_rain.sampleRate;
    ...
    lowPassFilter1.frequency.setValueAtTime(400, audioCtx_rain.currentTime);


    rain_slider.addEventListener("input", function() {
        gainNodeRain.gain.value = this.value * 10000;
    });
...
function cricket(){
    ...
    cricket_slider.addEventListener("input", function () {
        // Adjust the gain (loudness) based on the slider value
...
function get_slider_value(){
    ...
    if (!white_slider.disabled) {
        rate += (white_slider.value);
    }
    if (!ice_slider.disabled) {
        rate += (ice_slider.value);
    }
    if (!brook_slider.disabled) {
        rate += (brook_slider.value);
    }
    if (!fire_slider.disabled) {
        rate += (fire_slider.value);
    }
    if (!ocean_slider.disabled) {
        rate += (ocean_slider.value);
    }
    if (!wind_slider.disabled) {
        ate += (wind_slider.value);
    }
    if (!rain_slider.disabled) {
        rate += (rain_slider.value);
    }
    if (!cricket_slider.disabled) {
        rate += (cricket_slider.value);
    }
